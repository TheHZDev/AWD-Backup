...
-- 导出 ecshop 的数据库结构
CREATE DATABASE IF NOT EXISTS `ecshop` /*!40100 DEFAULT CHARACTER SET utf8 */;
USE `ecshop`;

-- 导出  表 ecshop.ecs_account_log 结构
...
-- 正在导出表  ecshop.ecs_admin_action 的数据：109 rows
...
-- 正在导出表  ecshop.ecs_admin_message 的数据：0 rows
...
-- 正在导出表  ecshop.ecs_admin_user 的数据：1 rows
...
INSERT INTO `ecs_admin_user` (`user_id`, `user_name`, `email`, `password`, `ec_salt`, `add_time`, `last_login`, `last_ip`, `action_list`, `nav_list`, `lang_type`, `agency_id`, `suppliers_id`, `todolist`, `role_id`) VALUES
	(1, 'admin', 'admin@qq.com', '0192023a7bbd73250516f069df18b500', NULL, 1536103842, 0, '', 'all', '商品列表|goods.php?act=list,订单列表|order.php?act=list,用户评论|comment_manage.php?act=list,会员列表|users.php?act=list,商店设置|shop_config.php?act=list_edit', '', 0, 0, NULL, NULL);
/*!40000 ALTER TABLE `ecs_admin_user` ENABLE KEYS */;

-- 导出  表 ecshop.ecs_adsense 结构
...
-- 正在导出表  ecshop.ecs_affiliate_log 的数据：0 rows
...
-- 正在导出表  ecshop.ecs_article 的数据：6 rows
...
INSERT INTO `ecs_article` (`article_id`, `cat_id`, `title`, `content`, `author`, `author_email`, `keywords`, `article_type`, `is_open`, `add_time`, `file_url`, `open_type`, `link`, `description`) VALUES
	(1, 2, '免责条款', '', '', '', '', 0, 1, 1536132642, '', 0, '', NULL),
	(2, 2, '隐私保护', '', '', '', '', 0, 1, 1536132642, '', 0, '', NULL),
	(3, 2, '咨询热点', '', '', '', '', 0, 1, 1536132642, '', 0, '', NULL),
	(4, 2, '联系我们', '', '', '', '', 0, 1, 1536132642, '', 0, '', NULL),
	(5, 2, '公司简介', '', '', '', '', 0, 1, 1536132642, '', 0, '', NULL),
	(6, -1, '用户协议', '', '', '', '', 0, 1, 1536132642, '', 0, '', NULL);
/*!40000 ALTER TABLE `ecs_article` ENABLE KEYS */;

-- 导出  表 ecshop.ecs_article_cat 结构
...
-- 正在导出表  ecshop.ecs_article_cat 的数据：3 rows
...
-- 正在导出表  ecshop.ecs_attribute 的数据：210 rows
...
-- 导出  表 ecshop.ecs_auction_log 结构
...
-- 正在导出表  ecshop.ecs_booking_goods 的数据：0 rows
...
-- 正在导出表  ecshop.ecs_cat_recommend 的数据：0 rows
...
-- 导出  表 ecshop.ecs_collect_goods 结构
...
-- 正在导出表  ecshop.ecs_collect_goods 的数据：0 rows
...
-- 正在导出表  ecshop.ecs_delivery_goods 的数据：0 rows
...
-- 导出  表 ecshop.ecs_delivery_order 结构
...
-- 正在导出表  ecshop.ecs_delivery_order 的数据：0 rows
...
-- 正在导出表  ecshop.ecs_email_sendlist 的数据：0 rows
...
-- 正在导出表  ecshop.ecs_exchange_goods 的数据：0 rows
...
-- 导出  表 ecshop.ecs_favourable_activity 结构
...
-- 正在导出表  ecshop.ecs_favourable_activity 的数据：0 rows
...
-- 正在导出表  ecshop.ecs_friend_link 的数据：3 rows
...
-- 正在导出表  ecshop.ecs_goods_activity 的数据：0 rows
...
-- 导出  表 ecshop.ecs_goods_article 结构
...
-- 正在导出表  ecshop.ecs_goods_article 的数据：0 rows
...
-- 正在导出表  ecshop.ecs_goods_gallery 的数据：0 rows
...
-- 正在导出表  ecshop.ecs_goods_type 的数据：9 rows
...
-- 导出  表 ecshop.ecs_group_goods 结构
...
-- 正在导出表  ecshop.ecs_mail_templates 的数据：14 rows
...
-- 导出  表 ecshop.ecs_member_price 结构
...
-- 正在导出表  ecshop.ecs_member_price 的数据：0 rows
...
-- 正在导出表  ecshop.ecs_order_action 的数据：0 rows
...
-- 导出  表 ecshop.ecs_order_goods 结构
...
-- 正在导出表  ecshop.ecs_package_goods 的数据：0 rows
...
-- 正在导出表  ecshop.ecs_region 的数据：3,408 rows
...
-- 导出  表 ecshop.ecs_reg_extend_info 结构
...
-- 正在导出表  ecshop.ecs_reg_extend_info 的数据：0 rows
...
-- 正在导出表  ecshop.ecs_reg_fields 的数据：6 rows
...
-- 正在导出表  ecshop.ecs_searchengine 的数据：0 rows
...
-- 正在导出表  ecshop.ecs_sessions 的数据：1 rows
...
INSERT INTO `ecs_sessions` (`sesskey`, `expiry`, `userid`, `adminid`, `ip`, `user_name`, `user_rank`, `discount`, `email`, `data`) VALUES
	('310b3fe302c96d4cb5697cbbeb8f7868', 1536133079, 0, 0, '172.29.0.1', '0', 0, 1.00, '0', 'a:3:{s:7:"from_ad";i:0;s:7:"referer";s:6:"本站";s:10:"login_fail";i:0;}');
/*!40000 ALTER TABLE `ecs_sessions` ENABLE KEYS */;

-- 导出  表 ecshop.ecs_sessions_data 结构
...
-- 正在导出表  ecshop.ecs_sessions_data 的数据：0 rows
...
-- 正在导出表  ecshop.ecs_shipping_area 的数据：0 rows
...
-- 导出  表 ecshop.ecs_shop_config 结构
...
-- 正在导出表  ecshop.ecs_shop_config 的数据：175 rows
...
INSERT INTO `ecs_shop_config` (`id`, `parent_id`, `code`, `type`, `store_range`, `store_dir`, `value`, `sort_order`) VALUES
	...
	(605, 6, 'install_date', 'hidden', '', '', '1536132643', 1),
...
-- 正在导出表  ecshop.ecs_stats 的数据：1 rows
...
INSERT INTO `ecs_stats` (`access_time`, `ip_address`, `visit_times`, `browser`, `system`, `language`, `area`, `referer_domain`, `referer_path`, `access_url`) VALUES
	(1536104279, '172.29.0.1', 1, 'FireFox 61.0', 'Unknown', 'zh-CN,zh', 'LAN', '', '', '/index.php');
/*!40000 ALTER TABLE `ecs_stats` ENABLE KEYS */;

-- 导出  表 ecshop.ecs_suppliers 结构
...
-- 正在导出表  ecshop.ecs_template 的数据：22 rows
...
-- 正在导出表  ecshop.ecs_user_account 的数据：0 rows
...
-- 导出  表 ecshop.ecs_user_address 结构
...
-- 正在导出表  ecshop.ecs_user_address 的数据：0 rows
...
-- 正在导出表  ecshop.ecs_user_rank 的数据：1 rows
...
-- 导出  表 ecshop.ecs_virtual_card 结构
...
-- 正在导出表  ecshop.ecs_virtual_card 的数据：0 rows
...
-- 导出  表 ecshop.ecs_volume_price 结构
...
-- 正在导出表  ecshop.ecs_volume_price 的数据：0 rows
...
-- 正在导出表  ecshop.ecs_wholesale 的数据：0 rows
